...
def parse_line(line):
    ...
    # result[pic_path].append({'x1': x1, 'x2': x2, 'y1': y1, 'y2': y2, 'class': class_name})
...
def parse_data(line, class_num, img_size, anchors, mode, letterbox_resize):
    ...
    return img_idx, img, boxes, labels


def get_batch_data(batch_line, class_num, img_size, anchors, mode='train', multi_scale=True, mix_up=False, letterbox_resize=True, interval=10):
    '''
    ...
    for line in batch_line:
        ...
        img_idx , img, boxes, labels = parse_data(line, class_num, img_size, anchors, mode, letterbox_resize)
    ...
    img_idx_batch, img_batch, boxes_batch, labels_batch = np.asarray(img_idx_batch, np.int64), np.asarray(img_batch), np.asarray(boxes_batch), np.asarray(labels_batch, np.int64))

    return img_idx_batch, img_batch, boxes_batch, labels_batch
