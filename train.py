...
train_dataset = train_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         ...
                         Tout=[tf.int64, tf.float32, tf.float32, tf.int64]),
...
image_ids, image, boxes, labels = iterator.get_next()
y_true = [boxes, labels]
...
# Model definition
##################
retinanet_model = retinanet(args.class_num, args.anchors, args.use_label_smooth, args.use_focal_loss, args.batch_norm_decay, args.weight_decay)
with tf.variable_scope('retinanet'):
    pred_feature_maps = retinanet_model.forward(image, is_training=is_training)
loss = retinanet_model.compute_loss(pred_feature_maps, y_true)
y_pred = retinanet_model.predict(pred_feature_maps)
